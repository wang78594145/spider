...
slog_path = os.path.dirname(os.path.dirname(__file__)) + '/docs/spider.log'
log = Logger(slog_path, level='info')
...
def deal_conn_exception(post_url, payload, header, proxies,ip_list):
    ...
    for i in range(10):
        ...
        if i == 9:
            ...
            ip_remove = proxies["http"].replace("http://","")
            log.logger.debug("ip_remove"+ip_remove)
            if ip_remove in ip_list:
                ip_list.remove(ip_remove)
            if len(ip_list) == 0:
                if excute_main(ipool_path):
                    ip_list = get_ip_pool(ip_list)
                    # get_proxies(target_url, ip_list)
            ...
            try:
                list = json.loads(
                    requests.post(post_url, data=json.dumps(payload), proxies=proxies, headers=header,timeout=timeout).content.decode("utf-8"))
                return list
            except:
                # 递归处理post抛出来的异常，直到返回当前查询数据为止，否则会丢数据或者程序退出。
                list = deal_conn_exception(post_url, payload, header, proxies,ip_list)
                return list
            # list = json.loads(
            #     requests.post(post_url, data=json.dumps(payload),
...
def  check_order_date( input_date):
    ...
    if(diff_time<0 or diff_time>90*24*60*60):
        log.logger.warning("输入的预约租车时间应该在当前日期后90天内,按照默认明天13:30:00作为预约时间")
        return False
...
def spidier_start(csv_path):
    ...
    input_time = input("输入预约租车时间（格式：2019-01-01）: ");
    check_result = check_order_date(input_time)
            ...
            for area in alist:
                ...
                for lllist in llist:
                    ...
                    if (vehicle_list["responseStatus"]["Ack"] == "Success"):
                        ...
                        #部分地区没有 租车信息,直接跳过本次循环开启下一次循环
                        if not carinfo_list:
                            log.logger.debug("dict为空跳过本次循环")
                            continue
